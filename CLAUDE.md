...
- **Astro 7** (upgraded from 5.18 on 25/09/2026), static, deployed by
  `.github/workflows/deploy.yml` on every push to `main` (plus
  `workflow_dispatch`). Source = "GitHub Actions" in repo settings. Custom
  domain via `public/CNAME`. Two settings in `astro.config.mjs` keep the
  output identical to Astro 5 — verified page by page against a 5.18 build:
  `compressHTML: true` (Astro 7's default `"jsx"` eats the space between
  inline elements on separate lines, which would glue words in the letters)
  and `build.cssMinify: "esbuild"` (Lightning CSS rewrote rules and dropped
  `-webkit-backdrop-filter`). Astro component `<style>`s still go through
  Lightning CSS: a prefixed/unprefixed pair in one block loses one of the
  two, so put the prefixed one in its own rule (see `VoltarInicio.astro`).
...
- **`src/layouts/BaseLayout.astro`** is the document shell. Props:
  ...
  "Para a Mi — 30/09/<ano>".
- **2023 and 2024 were hand-written HTML in `public/<ano>/`** until
  26/09/2026 (REPO-08); now they are letters like any other, in
  `src/anos/2023/` and `src/anos/2024/`, with `BaseLayout`, `VoltarInicio`
  and `NavAnos`. Their markup, CSS and scripts moved as they were (the old
  `<style>` and `2024.css` became `estilo.css`), and both render
  pixel-identical to the HTML versions. "Preserved" means the text and the
  concept, not the CSS: both had a "beautification" pass, and on 25/09/2026
  readability fixes. Both URLs still work: `/2023/` and `/2023/index.html`
  are the same file. 2014/2015/2020 are pages generated by
  `src/pages/[video]/index.astro`: the YouTube video embedded plus the
  transcript. The transcripts live in `src/data/transcricoes/<ano>.txt`, not
  in `public/`, because they carry working notes in `[colchetes]` meant only
  for him and the AI. `src/data/transcricoes.ts` strips every `[...]` before
  the text reaches the page or `/<ano>/video-text.txt` (generated by
  `src/pages/[video]/video-text.txt.ts`, at the URL the file always had).
...
- **Node 22.12+** (`.nvmrc`, `engines`; Astro 7's minimum), the same as the
  deploy.
- **Tailwind is frozen at 3.4.19** (exact versions in `package.json`). Do not
  run `npm update` blindly and do not move to Tailwind 4:
  `tailwind.config.mjs` uses `require` in an ESM file, which only Tailwind 3's
  loader tolerates.
- **`EPERM … unlink dist\…` on `npm run build`** means some process still has
  a file in `dist/` open — in September 2026 it was a forgotten
  `astro preview` from hours earlier. Stop it (or `npm run dev:fresh`, which
  frees 4321/4322) and build again. The `DEP0147 fs.rmdir` warning printed
  with it comes from Astro's retry path and disappears with the lock.

### Constraints found in the repo-hygiene PR (for Adriano to review)

- **Local Node (REPO-10)** was 20.16; by 25/09/2026 it is 22.23, which also
  covers Astro 7's 22.12 minimum.
...
- **"← início" pill (UX-10)**: kept `position: fixed`, but it slides away
  ...
  Since 25/09 it is one component, `src/components/VoltarInicio.astro`, on
  2025, 2026 and the video pages (2026 and the videos had a plain italic
  link, and he asked for the same control everywhere). A page that uses it
  must add `family=Lobster` to its `fontsHref`. Since REPO-08 (26/09) 2023
  and 2024 use it too; no hand copies are left.
...
- **Sharing tags on 2023/2024**: those pages had none (a WhatsApp link to
  them showed no preview). First a hand-made copy of BaseLayout's; since
  REPO-08 (26/09) they come from BaseLayout itself.
...
### REPO-08 (2023 and 2024 into Astro), 26/09/2026

- **Pixel-identical, verified**: full-page screenshots at 390px and 1440px
  of both pages, before and after, compared pixel by pixel in Chromium —
  0.000% difference, same page heights. With motion on: the pill hides and
  returns, 2023's 18 reveal paragraphs all show, hearts and petals spawn,
  2024's parallax moves and its closing line ends fully visible.
- **To stay identical, `NavAnos` gained two opt-in CSS variables**, with the
  old values as defaults so no other page changes: `--nav-anos-cor` (2023
  keeps its `#9B3358`) and `--nav-anos-topo` (2024 keeps the nav 1.5rem below
  "Dri", on its blue band — `.nav-fundo` in `src/anos/2024/estilo.css`).
- **Images are imported now** (`fotos/` and `ilustracoes/`), so their old
  URLs (`/2023/mi.png`, `/2024/image01.png`, `/2024/2024.css`…) are gone;
  nothing linked to them. Vite inlines the small wave SVGs of 2024 into the
  CSS as data URIs.
- **The scripts became bundled `<script>`s** (module, deferred) instead of
  inline ones at the end of `<body>`; they run at the same moment in
  practice and the code is unchanged, still ES5 `var`/IIFE as written.
- **The gallery links are `/2023/` and `/2024/`** now (`anos.ts`), like the
  other years; `tipo: "legado"` is gone, both are `"carta"`.
- **`canonical`/`og:url` changed** from `/<ano>/index.html` to `/<ano>/`,
  which is what BaseLayout emits for every page.
